...
# One shared limiter across ALL client instances in this process. The scorer, the
# walk-forward report, and the live-copy market-data loop each build a client; a
# per-instance limiter let their combined REST traffic exceed HL's per-IP weight cap.
_GLOBAL_LIMITER = RateLimiter(C.RATE_LIMIT_PER_MIN)
...
class HyperliquidClient:
    ...
    def discover_universe(self, extra_seeds: list[str] | None = None,
        ...
        Returns addresses in LEADERBOARD RANK ORDER (top performers first), then
        seeds — so the auditor scores the most promising wallets first.
        """
        ordered = self.leaderboard_addresses(leaderboard_url)
        seen = set(ordered)
    ...
    def leaderboard_addresses(self, leaderboard_url: str = C.HL_LEADERBOARD,
                              window: str = "month") -> list[str]:
        """
        Top traders from Hyperliquid's public leaderboard, ranked by PnL in the
        chosen window (day|week|month|allTime). These performers are the prime
        audit candidates — far better signal than raw trade-stream activity,
        which over-samples market-makers and bots.
        """
        try:
            ...
            print(f"[discover] leaderboard fetch failed ({e}); using harvested wallets only")
            return []
        rows = data.get("leaderboardRows", data if isinstance(data, list) else [])

        def pnl_for(row):
            for w, perf in (row.get("windowPerformances") or []):
                if w == window:
                    try:
                        return float(perf.get("pnl", 0) or 0)
                    except (TypeError, ValueError):
                        return 0.0
            return 0.0

        try:
            rows = sorted(rows, key=pnl_for, reverse=True)
        ...
        print(f"[discover] leaderboard returned {len(out)} ranked wallets")
        return out

    def portfolio(self, address: str) -> list:
        """Account-value + PnL history across time windows. Used to compute REAL
        leverage (trade notional vs account equity at the time of the trade)."""
        return self._post({"type": "portfolio", "user": address}) or []
