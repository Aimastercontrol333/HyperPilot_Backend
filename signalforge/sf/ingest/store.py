...
def rotate_unscored_first(conn: sqlite3.Connection, candidates: list[str],
    ...
    """Spread audits across the universe: audit NEVER-scored candidates first,
    then those scored longest ago (older than restale_hours), so every pass
    explores new wallets instead of re-auditing the same top names forever."""
    rows = conn.execute("SELECT address, scored_at FROM wallet_scores").fetchall()
    scored_at = {r["address"]: r["scored_at"] for r in rows}
    now = int(time.time() * 1000)
    stale_cutoff = now - restale_hours * 3600 * 1000
    never, stale, fresh = [], [], []
    for a in candidates:
        if a not in scored_at:
            never.append(a)
        elif scored_at[a] < stale_cutoff:
            stale.append(a)
        else:
            fresh.append(a)
    stale.sort(key=lambda a: scored_at[a])           # oldest first
