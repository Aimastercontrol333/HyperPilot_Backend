...
def realign_eligibility(conn: sqlite3.Connection, gate: dict) -> int:
    """Demote any stored-eligible wallet that no longer clears the current COPY gate
    (trades, history, single-trade loss, drawdown, leverage, sharpe, expectancy), using
    the metrics already saved with each score — so the basket reflects today's rules
    immediately instead of waiting for the slow re-audit sweep. Only demotes; promotions
    still happen through normal re-scoring. A criterion whose stored value is missing is
    skipped (not blocked on), so wallets are never demoted for data we don't have."""
    rows = conn.execute(
        "SELECT address, n_trades, max_dd, expectancy, data_json FROM wallet_scores "
        "WHERE eligible=1 AND banned=0"
    ).fetchall()
    demote = []
    for r in rows:
        try:
        ...
        fails = (
            nt < gate["min_trades"]
            or (hist is not None and hist < gate["min_history_days"])
            or (msl is not None and msl >= gate["max_single_trade_loss_pct"])
            or (dd is not None and dd >= gate["max_drawdown_pct"])
