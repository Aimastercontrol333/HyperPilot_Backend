...
def run(seeds: list[str], lookback_days: int = C.LOOKBACK_DAYS,
    ...
    # Discovery order = LEADERBOARD FIRST (top performers — the gold mine),
    # then seeds, then harvested trade-stream wallets. We audit the most
    # promising candidates before spending budget on the long tail.
    conn = None
    universe: list[str] = []
    ...
    if db_path:
        ...
        for a in store.top_addresses(conn, limit=max_wallets or 500, min_hits=2):
            if a not in seen:
                universe.append(a); seen.add(a)
    if max_wallets:
        universe = universe[:max_wallets]
    print(f"[pipeline] auditing {len(universe)} wallets "
          f"(leaderboard-first{'+harvested' if db_path else ''})...")
