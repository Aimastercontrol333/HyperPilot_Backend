...
def analyze(fills_by_wallet: dict[str, list[dict]], boundary_ms: int,
    ...
    rows: list[_WFRow] = []
    for addr, fills in fills_by_wallet.items():
        r = row_for_wallet(fills, av_by_wallet.get(addr), boundary_ms)
        if r is not None:
            rows.append(r)
...
def row_for_wallet(fills: list[dict], av: list | None, boundary_ms: int) -> "_WFRow | None":
    ...
    if len(train) < C.MIN_TRADES or len(test) < MIN_TEST_TRADES:
        return None
    train_fills = [f for f in fills if int(f["time"]) <= boundary_ms]
    av_tr = [(ts, v) for ts, v in (av or []) if ts <= boundary_ms]
    m = compute_metrics("", train, av_history=av_tr, fills=train_fills)
    sr = score_wallet(m)
    if sr is None:
        return None
    return _WFRow(sr.score, sr.raw_score, sr.banned, sr.eligible, _forward_outcomes(test))
