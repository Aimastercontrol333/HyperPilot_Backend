...
def walk_forward(fills_by_wallet: dict[str, list[dict]], boundary_ms: int,
                 account_values: dict[str, float] | None = None) -> list[DecileResult]:
    """
    boundary_ms splits each wallet's history into train (score) / test (forward).
    Returns decile buckets of (avg score) vs (avg forward net return).
    """
    ...
    for addr, fills in fills_by_wallet.items():
        ...
        if len(train) < C.MIN_TRADES or not test:
            continue
        m = compute_metrics(addr, train, account_values.get(addr))
        sr = score_wallet(m)
        if sr is None:
            continue
