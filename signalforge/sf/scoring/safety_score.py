...
def _f_leverage(m: WalletMetrics) -> float:
    # only meaningful when leverage is real (not proxy); proxy -> neutral-ish
    if m.extra.get("leverage_is_proxy"):
        return 0.5
...
def _auto_ban(m: WalletMetrics) -> list[str]:
    ...
    if (not m.extra.get("leverage_is_proxy")) and m.max_leverage_proxy >= C.AUTO_BAN["max_leverage_ever"]:
...
def _auto_pass(m: WalletMetrics) -> bool:
    ...
    return (
        ...
        and (m.extra.get("leverage_is_proxy") or m.avg_leverage_proxy < ap["avg_leverage"])
...
def score_wallet(m: WalletMetrics) -> ScoreResult | None:
    ...
    eligible = (not banned) and _auto_pass(m)
    ...
    return ScoreResult(
        ...
        contributions={k: round(v, 2) for k, v in contrib.items()}, metrics=m,
    )
