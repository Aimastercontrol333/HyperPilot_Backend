...
def _expectancy_gate(m) -> float:
    """Minimum edge to be copyable: the GREATER of the flat floor and our estimated cost
    to follow this wallet plus a margin. Closes the hole where a wallet beats the flat
    floor on paper yet still loses us money after the spread/slippage/fees WE pay."""
    ap = C.AUTO_PASS
    cost_floor = (getattr(m, "est_follower_cost_pct", 0.0) or 0.0) + ap.get("cost_margin_bps", 0.0) / 100.0
    return max(ap.get("min_expectancy_pct", 0.0), cost_floor)
...
def score_wallet(m: WalletMetrics) -> ScoreResult | None:
    ...
    if m.extra.get("too_fast"):
        notes.append("uncopyable_latency_scalper")
