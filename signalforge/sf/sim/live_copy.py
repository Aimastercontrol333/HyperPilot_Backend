...
class PaperPortfolio:
    ...
    def _open(self, trader, coin, side, mark_px, ts):
        ...
        cap = C.PORTFOLIO.get("stop_cap_pct", 12.0)
        if vol_mult and vol:
            # We HAVE volatility data: scale the stop to it.
            stop_pct = min(max(self.own_stop_pct, vol_mult * vol), cap)
        else:
            # NO vol data (almost always a builder-dex synthetic — stocks, oil, gold,
            # pre-IPO — whose candles we can't fetch). These are the MOST volatile coins,
            # so the old "fall back to the 5% floor" was exactly backwards and caused ~92%
            # of realized losses. Fall back to a sensible WIDE default by coin class.
            prefixes = C.PORTFOLIO.get("builder_dex_prefixes", ("xyz:", "vntl:"))
            if any(coin.startswith(p) for p in prefixes):
                stop_pct = C.PORTFOLIO.get("stop_fallback_builder_pct", 9.0)
            else:
                stop_pct = C.PORTFOLIO.get("stop_fallback_other_pct", 6.5)
