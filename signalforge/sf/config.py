...
# ----------------------------------------------------------------------------
MIN_HISTORY_DAYS = 90         # auto-pass needs >= this much history
PREFERRED_TRADES = 200        # confidence ramps toward this many closed trades
MIN_TRADES = 30               # below this we don't even score (pure noise)
...
SHRINKAGE_K = 60              # ~ "trades needed to half-trust the raw score"
...
FILL = {
    "detection_latency_s": 2.0,   # WS fill -> your ingestion (be honest, not 0)
    "decision_latency_s": 0.5,    # normalize + size + route
    "half_spread_bps": {          # crossing the book, per side, by liquidity tier
        "major": 1.5,             # BTC, ETH
    ...
    "delay_drift_bps": 8.0,       # placeholder until replay-measured; applied adversely
    "taker_fee_bps": 3.5,         # per side; HL taker ~0.025-0.035%
}

# Funding accrues while a position is open. We pull real funding history per coin;
# this is only the fallback if history is missing.
FUNDING_FALLBACK_BPS_PER_8H = 1.0
